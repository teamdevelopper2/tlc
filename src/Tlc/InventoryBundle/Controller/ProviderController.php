...
use Tlc\InventoryBundle\Entity\Provider;
...
class ProviderController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $providers = $em->getRepository('InventoryBundle:Provider')->findAll();
    ...
    public function newAction(Request $request)
    {
        ...
        $form = $this->createForm('Tlc\InventoryBundle\Form\ProviderType', $provider);
    ...
    {
        $deleteForm = $this->createDeleteForm($provider);
        $editForm = $this->createForm('Tlc\InventoryBundle\Form\ProviderType', $provider);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('provider_index', array('id' => $provider->getId()));
        }
